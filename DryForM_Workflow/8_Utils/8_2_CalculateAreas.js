/**** AREA OF CLASS VALUE 0 USING EACH INPUT IMAGE EXTENT AS AOI
 *
 ...
 *   - The script calculates the area where class value == 0.
 *
 * Optional grid mode:
...
var GRID_SIZE_M = 200000;  // 50 km. Try 25000 or 10000 if needed.
...
// This controls only map previews and prints.
var SHOW_VIS = true;

// Display the grid layer when USE_GRID = true.
var SHOW_GRID = true;

// Export settings.
var EXPORT_FOLDER = 'GEE_area_exports';
var EXPORT_NAME = 'DryForM_class_0_area_by_dataset_image_extent';
...
//   - with your current list, this means 5 export tasks.
...
// Change this if any layer was exported at a different spatial resolution.
...
// ======================================================
//
// The script automatically selects the first band of each image.
// If needed, manually specify a band by changing band: null to,
// for example, band: 'classification' or band: 'b1'.
...
// ======================================================
//
// Several datasets may belong to the same AOI.
// For example, Caatinga has three models.
// We only need to display the Caatinga AOI/grid once.
...
function getImageAOI(cfg) {
  ...
  // Bounding box of image footprint.
  var bbox = footprint.bounds(1);

  var aoi = USE_BOUNDING_BOX ? bbox : footprint;
...
// Creates a binary mask for class value 0.
// Pixels equal to CLASS_VALUE become 1.
// Other pixels are masked.
function makeClassMask(cfg) {
  var classImage = getClassImage(cfg);

  var classMask = classImage
    .eq(CLASS_VALUE)
    .selfMask()
    .rename('class_mask');
...
// Creates an area image in m² for class value 0.
function makeAreaImage(cfg) {
  var areaImage = makeClassMask(cfg)
...
// Creates an area image representing valid data coverage.
// This is useful because the AOI may be the image bounding box,
// but not all pixels inside the box are necessarily valid.
...
// This is calculated only once per AOI name, not once per model.
...
function calculateWholeImageAOI(cfg) {
  ...
  var countImage = makeClassMask(cfg);
  ...
  var classAreaM2 = zeroIfNull(stats.get('area_m2'));
  var pixelCount = zeroIfNull(stats.get('class_mask'));
  ...
  var percentOfAOI = ee.Number(
    ee.Algorithms.If(
      aoiAreaM2.gt(0),
      classAreaM2.divide(aoiAreaM2).multiply(100),
      0
    )
  );
  ...
  var percentOfValidData = ee.Number(
    ee.Algorithms.If(
      ...
      classAreaM2.divide(validDataAreaM2).multiply(100),
      0
    )
  );
  ...
  return ee.Feature(null, {
    ...
    'grid_size_m': 0,
    'number_of_tiles': 0,
    'class_area_m2': classAreaM2,
    'class_area_ha': classAreaM2.divide(1e4),
    'class_area_km2': classAreaM2.divide(1e6),
    ...
    'pixel_count_class_value': pixelCount
  });
}
...
function calculateImageAOIWithGrid(cfg) {
  ...
  var classAreaM2 = ee.Number(tileResults.aggregate_sum('class_area_m2'));
  var pixelCount = ee.Number(tileResults.aggregate_sum('pixel_count_class_value'));
...
if (SHOW_VIS) {
  ...
  // Add each class-0 mask as a separate layer.
  // They are turned off by default to avoid visual clutter.
  DATASETS.forEach(function(cfg) {
    Map.addLayer(
      makeClassMask(cfg),
      {palette: ['blue']},
      'Class 0 - ' + cfg.dataset_id,
      false
    );
  });
